...
class Day1Test {
    ...
    them is 2.
    The second-smallest number in the left list is 2, and the second-smallest number in the right list is
    ...
    @Test
    fun `Part1, Should find diff`() {
        val left = listOf(3, 4, 2, 1, 3, 3)
        val right = listOf(4, 3, 5, 3, 9, 3)
        val expectedDiff = 11
        val actual = Day1.solvePart1(left, right)
        assertThat(actual).isEqualTo(expectedDiff)
    }

    @Test
    fun `Part1 Solution, Should find diff for real input`() {
        val left = mutableListOf<Int>()
        val right = mutableListOf<Int>()
        CsvReader.readRowsAsInt("day1/part1-input.csv")
            .forEach { row ->
                left.add(row[0])
                right.add(row[1])
            }
    ...
    @Test
    fun `Part2, Should find similarity score`() {
        val left = listOf(3, 4, 2, 1, 3, 3)
        val right = listOf(4, 3, 5, 3, 9, 3)
        val expectedScore = 31
        val actual = Day1.solvePart2(left, right)
        assertThat(actual).isEqualTo(expectedScore)
    }

    @Test
    fun `Part2 Solution, Should find similarity score`() {
        val left = mutableListOf<Int>()
        val right = mutableListOf<Int>()
        CsvReader.readRowsAsInt("day1/part1-input.csv")
            .forEach { row ->
